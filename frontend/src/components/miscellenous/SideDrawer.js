  ...
  const handleSearch = async (value) => {
   if(!value){
    ...
    try {
      ...
      const { data } = await axios.get(`http://localhost:4000/v1/user/alluser?search=${value}`, config);

      setloading(false);
      if(data.user.length===0){
        let array=[{key:"notfound"}]
...
onClose()
  ...
  return (
    <>
      ...
      <Drawer placement="left" onClose={customOnClose} isOpen={isOpen}>
        <DrawerOverlay />
        <DrawerContent>
